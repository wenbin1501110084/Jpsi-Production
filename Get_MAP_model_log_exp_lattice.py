...
tot_error_col_dsigma2 = dsigmadata_select[:,3] * dsigmadata_select[:,3] / dsigmadata_select[:,2]/dsigmadata_select[:,2]
...
for iev in range(1):
    ...
    chi2 = (y_arr -y_arr_data )**2 / diagonal_elements
    sumchi2 = np.mean(chi2)
mapindex = 0
#for iev in range(len(sample)):
RR = random.sample(range(6000000), 1000000)
iii = 0
for iev in range(0,len(sample)):
    #print(sample[iev,:])
    Ag0 = sample[iev,0]
    MAg = sample[iev,1]
    Cg0 = sample[iev,2]
    ...
    y_arr = np.concatenate((y_arr, LDg_mean))
    chi2 = (y_arr -y_arr_data )**2 / diagonal_elements
    sumchi2temp = np.mean(chi2)
    if (sumchi2temp < sumchi2):
        sumchi2 = sumchi2temp
        mapindex = iev
    #iii = iii +1
    if (iev % 10000 == 0):
        print("MAP parameter is: ",iev,  sample[mapindex])
        print("Chi2 is : ", sumchi2**0.5)
print("MAP parameter is: ", sample[mapindex])
print("Chi2 is : ", sumchi2**0.5)
"""

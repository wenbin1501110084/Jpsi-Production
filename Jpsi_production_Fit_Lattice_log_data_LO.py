...
tot_error_col_dsigma2 = dsigmadata_select[:,3] * dsigmadata_select[:,3]/np.log(dsigmadata_select[:,2])/np.log(dsigmadata_select[:,2])
...
y_arr = np.log(dsigmadata_select[:,2])
...
x_arr = np.concatenate((x_arr, minus_t_D))
"""
np.savetxt("JPsi_data_lattice_log_exp.txt",
        np.array([x_arr, y_arr]).transpose(),
        fmt="%.6e", delimiter="  ",
        header="x  results")

np.savetxt("JPsi_data_cov_matrix_lattice_log_exp.txt",
        direct_sum_matrix,
        fmt="%.6e", delimiter="  ",
        header="x  results")
"""
...
for iev in range(1000):
    directory_name = "Bayesian_folder_Lattice_log_exp_data_LO/JPsi_{}".format(int(iev))
    try:
        os.mkdir(directory_name)
    except FileExistsError as e:
        iiiii = 1
    destination_path = "{}/parameter_{:03}".format(directory_name, int(iev))
    source_path = "../JPsi_Yuxun/main/parameter_{:03}".format(int(iev))
    ...
    Ag0 = parameters["A0Latg"]
    MAg = parameters["MALatg"]
    Cg0 = parameters["C0Latg"]
    MCg = parameters["MCLatg"]
    Aq0 = parameters["A0Latq"]
    MAq = parameters["MALatq"]
    Cq0 = parameters["C0Latq"]
    MCq = parameters["MCLatq"]
    
    shutil.copy(source_path, destination_path)
    
    t_spectra_data = dsigma_New(dsigmadata_select[:,0], -dsigmadata_select[:,1], Ag0, MAg ,Cg0, MCg, Aq0, MAq, Cq0, MCq, P_order = 1)

    y_arr = np.log(t_spectra_data)
    ...
    np.savetxt("Bayesian_folder_Lattice_log_exp_data_LO/JPsi_{}/Bayesian_output.txt".format(int(iev)),
        np.array([x_arr, y_arr, np.abs(y_arr)/100]).transpose(),
        fmt="%.6e", delimiter="  ",
        header="W  results  stat. err")

...
np.savetxt("JPsi_data_lattice_exp.txt",
        np.array([x_arr, y_arr]).transpose(),
        fmt="%.6e", delimiter="  ",
        header="x  results")

np.savetxt("JPsi_data_cov_matrix_lattice_exp.txt",
        direct_sum_matrix,
        fmt="%.6e", delimiter="  ",
        header="x  results")
...
for iev in range(1000):
    directory_name = "Bayesian_folder_Lattice_exp_data/JPsi_{}".format(int(iev))
    try:
    ...
    np.savetxt("Bayesian_folder_Lattice_exp_data/JPsi_{}/Bayesian_output.txt".format(int(iev)),
